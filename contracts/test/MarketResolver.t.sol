...
contract MockOptimisticOracle is IOptimisticOracleV2 {
    ...
    function requestPrice(
        ...
        bytes32 requestId = keccak256(
            abi.encodePacked(msg.sender, identifier, timestamp, ancillaryData)
        );
    ...
    function getState(
        ...
        if (block.timestamp >= requests[requestId].expirationTime)
...
contract MarketResolverTest is Test {
    ...
    ParimutuelPredictionMarket public platformMarket;
    ParimutuelPredictionMarket public userMarket;
    ...
    function setUp() public {
        ...
        platformMarket = new ParimutuelPredictionMarket(
            address(usdc),
            "Will BTC hit $100k?",
            endTime,
            address(resolver)
        ...
        userMarket = new ParimutuelPredictionMarket(
            address(usdc),
    ...
    function testRegisterMarket() public {
        ...
        ParimutuelPredictionMarket newMarket = new ParimutuelPredictionMarket(
            address(usdc),
            "Test market",
        ...
        assertEq(
            uint256(resolver.marketTypes(address(newMarket))),
            uint256(MarketResolver.MarketType.PLATFORM)
        );
    }
    ...
    function testPlatformMarketResolutionFlow() public {
        ...
        (
            ...
            bool resolved
        ) = resolver.pendingResolutions(address(platformMarket));
        ...
        mockOracle.mockSettlePrice(
            ...
            abi.encodePacked(
                "Market: Will BTC hit $100k?. Resolution: Should this market resolve to YES (1) or NO (0)?"
            ),
            1e18 // YES
        );
    ...
    function testCanResolveMarket() public {
        ...
        (bool canResolve, MarketResolver.MarketType resolutionType) = resolver
            .canResolveMarket(address(userMarket));
        assertFalse(canResolve);

        // Test after end time
        vm.warp(userMarket.endTime() + 1);
        (canResolve, resolutionType) = resolver.canResolveMarket(
            address(userMarket)
        );
        assertTrue(canResolve);
        assertEq(
            uint256(resolutionType),
            uint256(MarketResolver.MarketType.USER)
        );
    }
    ...
    function testOracleStateTracking() public {
        ...
        IOptimisticOracleV2.State state = resolver.getOracleState(
            address(platformMarket)
        );
