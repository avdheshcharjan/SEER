...
contract MockUSDC is ERC20, Ownable {
        ...
        if (lastRequest != 0) {
            ...
            if (timeSinceLastRequest < FAUCET_COOLDOWN) {
                revert FaucetCooldownActive(
                    FAUCET_COOLDOWN - timeSinceLastRequest
                );
            }
        }
    ...
    function getFaucetCooldown(
        ...
        // First-time users can use faucet immediately
        if (lastRequest == 0) {
            return 0;
        }
    ...
    function canUseFaucet(address user) external view returns (bool available) {
        ...
        return block.timestamp - lastRequest >= FAUCET_COOLDOWN;
    }
    ...
    function toHumanAmount(
        uint256 amount
    ) external pure returns (uint256 humanAmount) {
    ...
    function fromHumanAmount(
        uint256 humanAmount
    ) external pure returns (uint256 weiAmount) {
